...
"""2D harmonic analysis regression test for the chopped beam."""
...
class TestTimeDependentBeamHarmonics2D(unittest.TestCase):
    ...
    def test_time_dependent_beam_slice_harmonics(self):
        output_dir = os.path.join(
            os.path.dirname(__file__),
            'artifacts',
            'chopped_beam_harmonics_2d',
        )
        ...
        for frame_idx, time_point in enumerate(timestamps):
            ...
            # Zero the central x-line to simplify visual comparisons along the deflection axis
        ...
        npz_path = os.path.join(output_dir, 'chopped_beam_slice_fields.npz')
        np.savez_compressed(
            npz_path,
        ...
        fundamental_path = os.path.join(output_dir, 'chopped_beam_slice_fundamental.png')
        second_harmonic_path = os.path.join(output_dir, 'chopped_beam_slice_second_harmonic.png')
        third_harmonic_path = os.path.join(output_dir, 'chopped_beam_slice_third_harmonic.png')
        ...
        if num_times > 1 and sample_spacing > 0.0:
            ...
            ax_second_bx = fig_second.add_subplot(121)
            ax_second_bx.set_title('Second harmonic Bx amplitude')
            im_second_bx = ax_second_bx.imshow(
                second_bx.T,
                origin='lower',
                extent=extent,
                cmap='inferno',
            )
            fig_second.colorbar(im_second_bx, ax=ax_second_bx, fraction=0.046, pad=0.04, label='|Bx| [T]')
            ax_second_bx.set_xlabel('X [mm]')
            ax_second_bx.set_ylabel('Y [mm]')
            ...
            fig_third = self.plt.figure(figsize=(10, 5))
            ax_third_bx = fig_third.add_subplot(121)
            ax_third_bx.set_title('Third harmonic Bx amplitude')
            im_third_bx = ax_third_bx.imshow(
                third_bx.T,
                origin='lower',
                extent=extent,
                cmap='inferno',
            )
            fig_third.colorbar(im_third_bx, ax=ax_third_bx, fraction=0.046, pad=0.04, label='|Bx| [T]')
            ax_third_bx.set_xlabel('X [mm]')
            ax_third_bx.set_ylabel('Y [mm]')

            ax_third_by = fig_third.add_subplot(122)
            ax_third_by.set_title('Third harmonic By amplitude')
            im_third_by = ax_third_by.imshow(
                third_by.T,
                origin='lower',
                extent=extent,
                cmap='inferno',
            )
            fig_third.colorbar(im_third_by, ax=ax_third_by, fraction=0.046, pad=0.04, label='|By| [T]')
            ax_third_by.set_xlabel('X [mm]')
            ax_third_by.set_ylabel('Y [mm]')
            ...
            for destination, message in [
                ...
                fig_placeholder = self.plt.figure(figsize=(6, 4))
