"""Harmonic analysis regression test for the circularly modulated beam."""
...
class TestTimeDependentCircBeamHarmonics(unittest.TestCase):
    ...
    def test_time_dependent_circbeam_line_harmonics(self):
        output_dir = os.path.join(
            os.path.dirname(__file__),
            'artifacts',
            'oscillating_circbeam_harmonics',
        )
        ...
        fundamental_path = os.path.join(output_dir, 'oscillating_circbeam_line_fundamental.png')
        second_harmonic_path = os.path.join(output_dir, 'oscillating_circbeam_line_second_harmonic.png')
        ...
        if line_bx_arr.shape[0] > 1 and sample_spacing > 0.0:
            ...
            ax_fundamental.set_title(
                f'Circular beam line spectrum at x = {line_x_mm:.2f} mm (fundamental {DRIVING_FREQUENCY * 1e-6:.1f} MHz)'
            )
            ...
            ax_second.set_title(
                f'Circular beam line spectrum at x = {line_x_mm:.2f} mm (second harmonic {2.0 * DRIVING_FREQUENCY * 1e-6:.1f} MHz)'
            )
